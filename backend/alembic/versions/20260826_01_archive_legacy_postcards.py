...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "uq_postcards_trip_poi_kind_version", "postcards", type_="unique"
    )
    # Keep the newest rendering for each trip, POI, and artifact kind.
    op.execute(
        """
        ...
        WHERE older.trip_id = newer.trip_id
          ...
          AND older.artifact_kind = newer.artifact_kind
