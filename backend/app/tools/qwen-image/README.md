...
This copy uses the AgentScope 1.0 `ToolResponse` and `ImageBlock` API shipped
with QwenPaw 1.1.12. The downloaded plugin originally referenced the newer
`ToolChunk`, `ToolResultState`, and `DataBlock` names and passed a `tool_type`
argument unsupported by this QwenPaw release. No generation behavior or model
parameters were changed by the compatibility adaptation.
