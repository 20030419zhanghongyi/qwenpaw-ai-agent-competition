...
def test_route_import_is_idempotent_and_writes_database():
    ...
    assert first.templates_read == second.templates_read == 6
    assert first.stops_written == second.stops_written == 38
    assert first.legacy_ids_converted == second.legacy_ids_converted == 48
    assert second.inserted == 0
    assert second.updated == 6
    with SessionLocal() as session:
        assert session.scalar(select(func.count()).select_from(RouteTemplate)) == 6
        assert session.scalar(select(func.count()).select_from(RouteTemplateStop)) == 38
