...
const BACK_TO_MEMOIRS = {
  "zh-CN": "返回回忆录列表",
  "zh-TW": "返回回憶錄列表",
  en: "Back to memoirs",
  pt: "Voltar às memórias",
} as const;
...
export function MemoirEditorPage() {
  ...
  if (error && !memoir) return <main className="mx-auto max-w-3xl flex-1 px-5 py-12"><ErrorState title="Error" message={error} onRetry={() => navigate("/profile/memories")} retryLabel={BACK_TO_MEMOIRS[language]} /></main>;
  ...
  return <main className="relative flex-1 bg-paper pb-24"><div className="mx-auto max-w-3xl px-5 pt-8 lg:px-0">
    <Link to="/profile/memories" className="text-sm text-ink-soft">← {BACK_TO_MEMOIRS[language]}</Link>
    <div className="mt-6 overflow-hidden rounded-[2rem] border border-line bg-card shadow-[var(--shadow-soft)]">{coverUrl ? <img src={coverUrl} alt="" className="h-64 w-full object-cover" /> : <div className="h-40 bg-gradient-to-br from-sage-deep/20 via-paper-warm to-clay/10" />}<div className="p-6"><p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-sage-deep">{copy.eyebrow} · {memoir.trip_status === "completed" ? copy.finished : copy.ongoing}</p><label className="mt-4 block text-xs font-medium text-ink-soft">{copy.title}<input aria-label={copy.title} value={localizedMemoirTitle(memoir.title, language)} onChange={(e) => setMemoir({ ...memoir, title: e.target.value })} className="mt-2 w-full rounded-xl border border-line bg-paper px-4 py-3 font-display text-2xl text-ink outline-none transition-colors focus:border-sage-deep focus:ring-2 focus:ring-sage-deep/15" /></label><p className="mt-3 text-xs text-ink-soft">{memoir.travel_date ? new Date(memoir.travel_date).toLocaleDateString(language) : ""}</p></div></div>
    <AzulejoBand className="my-8" />
    {error ? <p className="mb-5 rounded-xl bg-clay/10 p-3 text-sm text-clay">{error}</p> : null}
    <section className="rounded-2xl border border-line bg-card p-5"><label className="block text-sm text-ink">{copy.intro}<textarea value={localizedMemoirIntroduction(memoir.introduction, language)} onChange={(e) => setMemoir({ ...memoir, introduction: e.target.value })} rows={3} className="mt-2 w-full rounded-xl border border-line bg-paper p-3" /></label></section>
