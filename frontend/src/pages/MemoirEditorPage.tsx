...
export function MemoirEditorPage() {
  ...
  useEffect(() => {
    ...
    void getMemoir(memoirId, token).then(setMemoir).catch((err: unknown) => setError(err instanceof Error ? err.message : "Unable to load memoir"));
  }, [memoirId, token]);
  ...
  const save = async (status: "draft" | "completed" = memoir?.status ?? "draft") => {
    if (!memoir || !token) return;
    setBusy(true); setError(null);
    try { setMemoir(await updateMemoir(memoir.memoir_id, { ...memoir, status }, token)); setSaved(true); window.setTimeout(() => setSaved(false), 1600); }
  ...
  return <main className="relative flex-1 bg-paper pb-24"><div className="mx-auto max-w-3xl px-5 pt-8 lg:px-0">
    ...
    <div className="sticky bottom-20 mt-8 flex flex-wrap justify-end gap-3 rounded-2xl border border-line bg-card/95 p-4 shadow-lg backdrop-blur"><button type="button" disabled={busy} onClick={() => void save("draft")} className="rounded-full border border-sage-deep px-5 py-2.5 text-sm text-sage-deep">{saved ? copy.saved : copy.save}</button><button type="button" disabled={busy} onClick={() => void save("completed")} className="rounded-full bg-sage-deep px-5 py-2.5 text-sm text-paper">{copy.complete}</button></div>
  </div></main>;
}
